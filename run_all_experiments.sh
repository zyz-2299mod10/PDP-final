...
#   2. Pure SP  (TP=2, SP=2, no shift, threshold=0)
...
run_exp "A2: Pure SP (TP=2, SP=2, no shift)" "A2_decode_pure_sp.log" \
...
for LONG_TOKENS in 1024 4096 8192 16384; do
    ...
    run_exp "B-${LONG_TOKENS}: Pure SP (TP=2, SP=2)" \
...
run_exp "C2: Pure SP (TP=2, SP=2)" "C2_balanced_pure_sp.log" \
    --model "$MODEL" --tp-size $TP_SIZE --sp-size $SP_SIZE \
    --threshold 0 --max-model-len $MAX_MODEL_LEN \
    --workload imbalance \
    --long-tokens 1024 --short-tokens 1024 --short-requests 31 \
