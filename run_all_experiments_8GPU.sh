...
#
# Runs FOUR modes:
#   1. Pure TP  (TP=8, SP=1, no shift)
#   2. Pure SP  (TP=1, SP=8, no shift, threshold=0)
#   3. Static Shift Parallel  (original threshold logic)
#   4. Adaptive Shift Parallel (our improvement)
#
# For V100 32GB กั 8 GPUs
# ============================================================
...
run_exp "A1: Pure TP (TP=8, SP=1)" "A1_decode_pure_tp.log" \
    --model "$MODEL" --tp-size 8 --sp-size 1 \
    --threshold 99999 --max-model-len $MAX_MODEL_LEN \
    --workload decode --tokens-count 500 \
    --no-shift 
    #--no-spec

run_exp "A2: Pure SP (TP=1, SP=8, no shift)" "A2_decode_pure_sp.log" \
    --model "$MODEL" --tp-size 1 --sp-size 8 \
...
for LONG_TOKENS in 1024 4096 8192 16384; do
    ...
    run_exp "B-${LONG_TOKENS}: Pure TP (TP=8, SP=1)" \
    ...
    run_exp "B-${LONG_TOKENS}: Pure SP (TP=1, SP=8)" \
...
run_exp "C1: Pure TP (TP=8, SP=1)" "C1_balanced_pure_tp.log" \
    --model "$MODEL" --tp-size 8 --sp-size 1 \
    --threshold 99999 --max-model-len $MAX_MODEL_LEN \
    --workload imbalance \
...
run_exp "C2: Pure SP (TP=1, SP=8)" "C2_balanced_pure_sp.log" \
    --model "$MODEL" --tp-size 1 --sp-size 8 \
    --threshold 0 --max-model-len $MAX_MODEL_LEN \
    --workload imbalance \
    --long-tokens 1024 --short-tokens 1024 --short-requests 31 \
...
SUMMARY_FILE="$RESULTS_DIR/8GPU_experiments_summary.log"

{
    echo ""
    echo "Quick summary:"
    echo "--------------------------------------------------------------"
    printf "%-45s %s\n" "8GPU Experiment" "Throughput"
    echo "--------------------------------------------------------------"
